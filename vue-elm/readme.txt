...
9.如果计算属性被相互引用计算，会导致死循环么？
会导致死循环。。。


10.父组件和子组件的数据传递问题？

父组件可以向子组件传递对象和数据，子组件修改对象后，父组件里面的对象数据自然也会改变
可是父组件如何调用子组件的方法？子组件如何调用父组件的方法呢？
本例中就是购物车小球动画的实现问题，两个不同组件如何传递数据进行组件联动？
...
1.如果子组件使用到store中的数据，需要放到computed里面 不然不会重新计算，使用的时候commit即可
